...
# =============================================================================
# EXIT ENGINE CONFIGURATION (Task 24)
# =============================================================================
# Professional Exit Engine - Zone-Aware, Asset-Class-Aware
# Replaces legacy fixed TP/SL with state-dependent logic

EXIT_ENGINE_DEFAULTS = {
    # Global Safety Settings
    'global': {
        'whale_threshold_price': 0.10,    # Entry price below this = Whale Zone
        'min_sell_size_usd': 2.00,        # Minimum sell size to avoid dust errors
        'max_spread_pct': 0.05,           # Do not exit if spread > 5% (wick protection)
        'expiry_guard_hours': 2.0,        # Force close losing trades 2h before expiry
    },
    
    # Base Strategy Time Horizons (in Hours)
    'strategy_max_hours': {
        'delta_neutral': 4,
        'arbitrage': 6,
        'volatility_exploitation': 24,
        'alpha_directional': 72,  # 3 Days
        'gamma_scalp': 168,       # 7 Days (whale positions held longer)
    },
    
    # Whale Zone Exit Rules (Entry < $0.10)
    # These are based on PRICE MULTIPLES, not percentages
    'whale_zone': {
    ...
    # Core Zone Exit Rules (Entry >= $0.10)
    'core_zone': {
        'base_take_profit_pct': 0.08,     # 8% base TP
        'base_stop_loss_pct': 0.05,       # 5% base SL
        'free_roll_trigger_pct': 0.30,    # Sell principal at +30%
        'trailing_stop_trigger_pct': 0.15,  # Enable trailing at +15%
        'trailing_stop_distance_pct': 0.10, # Trail 10% behind peak
        'breakeven_trigger_pct': 0.15,    # Move stop to breakeven at +15%
        'thesis_fail_time_pct': 0.50,     # Thesis fail at 50% of max hold time
        'zombie_check_hours': 24,         # Check for zombie after 24h
        'zombie_change_threshold_pct': 0.01,  # <1% change = zombie
    },
    
    # Asset Class Configuration
    # Controls HOW the exit engine treats each asset class
    'asset_config': {
        'politics': {
        ...
        'science': {
            'description': 'Binary/Lotto. Long holds.',
            'tp_mult': 1.0,
            'sl_mult': 1.0,
            'time_mult': 3.0,
            'use_trailing_stop': False,
        ...
        'crypto': {
            'description': 'High correlation assets.',
            'tp_mult': 1.5,
            'sl_mult': 1.3,
            'time_mult': 0.5,
            'use_trailing_stop': True,
        ...
        'entertainment': {
            'description': 'Pop culture events.',
            'tp_mult': 1.0,
            'sl_mult': 1.0,
            'time_mult': 1.0,
            'use_trailing_stop': True,
...
# Global Exit Engine instance (loaded from DB or defaults)
EXIT_ENGINE = dict(EXIT_ENGINE_DEFAULTS)


def get_exit_engine_config() -> Dict:
    """Get current exit engine configuration."""
    return EXIT_ENGINE


def update_exit_engine_config(new_config: Dict):
    """Update exit engine configuration in memory."""
    global EXIT_ENGINE
    
    if 'global' in new_config:
        EXIT_ENGINE['global'].update(new_config['global'])
    if 'strategy_max_hours' in new_config:
        EXIT_ENGINE['strategy_max_hours'].update(new_config['strategy_max_hours'])
    if 'whale_zone' in new_config:
        EXIT_ENGINE['whale_zone'].update(new_config['whale_zone'])
    if 'core_zone' in new_config:
        EXIT_ENGINE['core_zone'].update(new_config['core_zone'])
    if 'asset_config' in new_config:
        for asset, cfg in new_config['asset_config'].items():
            if asset in EXIT_ENGINE['asset_config']:
                EXIT_ENGINE['asset_config'][asset].update(cfg)
            else:
                EXIT_ENGINE['asset_config'][asset] = cfg
    
    logger.info("[EXIT ENGINE] Configuration updated")


def reset_exit_engine_config():
    """Reset exit engine to defaults."""
    global EXIT_ENGINE
    import copy
    EXIT_ENGINE = copy.deepcopy(EXIT_ENGINE_DEFAULTS)
    logger.info("[EXIT ENGINE] Reset to defaults")


def get_asset_exit_config(asset_class: str) -> Dict:
    """Get exit configuration for a specific asset class."""
    return EXIT_ENGINE['asset_config'].get(
        asset_class.lower(), 
        EXIT_ENGINE['asset_config']['default']
    )
