...
from ml.whale_tracker import whale_tracker as global_whale_tracker
...
class PaperTrader:
    ...
    def __init__(self, continuous_mode: bool = False, use_config_capital: bool = True):
        ...
        self.strategy_stats = {
            ...
            'hft_liquidity_provision': {'trades': 0, 'wins': 0, 'pnl': 0.0, 'gross_profit': 0.0, 'gross_loss': 0.0, 'total_hold_time': 0.0, 'closed_trades': 0},
            'hft_volatility_exploit': {'trades': 0, 'wins': 0, 'pnl': 0.0, 'gross_profit': 0.0, 'gross_loss': 0.0, 'total_hold_time': 0.0, 'closed_trades': 0},
            'hft_extreme_spread': {'trades': 0, 'wins': 0, 'pnl': 0.0, 'gross_profit': 0.0, 'gross_loss': 0.0, 'total_hold_time': 0.0, 'closed_trades': 0},
            'volatility_exploitation': {'trades': 0, 'wins': 0, 'pnl': 0.0, 'gross_profit': 0.0, 'gross_loss': 0.0, 'total_hold_time': 0.0, 'closed_trades': 0},
            'hft_sharp_following': {'trades': 0, 'wins': 0, 'pnl': 0.0, 'gross_profit': 0.0, 'gross_loss': 0.0, 'total_hold_time': 0.0, 'closed_trades': 0},
            'hft_delta_neutral': {'trades': 0, 'wins': 0, 'pnl': 0.0, 'gross_profit': 0.0, 'gross_loss': 0.0, 'total_hold_time': 0.0, 'closed_trades': 0},
        }
    ...
    async def _load_user_config(self):
        ...
        try:
            ...
            if user_config:
                ...
                logger.info(f"  Strategy Thresholds: Vol={self.volatility_threshold}, Sentiment={self.sentiment_strength_threshold}")
                logger.info(f"  Arbitrage Thresholds: MarketQuality>{self.market_quality_threshold}, SharpAlignment>{self.sharp_alignment_threshold}")
    ...
    async def _evaluate_exit(self, market_id: str, market_data: Dict):
        ...
        try:
            ...
            # there's no buyer. For sports markets, we MUST have:
            # 1. Fresh price from Polymarket API
            # 2. Verified orderbook with real bids/asks
            # 3. Exit price derived from actual orderbook, NOT API mid-price
            # 
            ...
            if strategy == 'sports_arbitrage' or asset_class == 'sports':
                try:
                    ...
                    async with PolymarketAPI() as api:
                        ...
                        if not fresh_market:
                            logger.warning(f"[SPORTS-EXIT-BLOCK] Market {market_id[:16]} not found - cannot exit without real price")
                            return
                        
                        token_ids = fresh_market.get('clobTokenIds', [])
                        if not token_ids:
                            logger.warning(f"[SPORTS-EXIT-BLOCK] No token IDs for {market_id[:16]} - cannot fetch orderbook")
                            return
                        
                        # Fetch REAL orderbook - this is the source of truth
                        side = position['side']
                        ...
                        if not bids and not asks:
                            logger.warning(f"[SPORTS-EXIT-BLOCK] Empty orderbook for {market_id[:16]} - no liquidity to exit")
                            return
                        
                        # ==========================================================
                        # REAL EXIT PRICE FROM ORDERBOOK
                        # ==========================================================
                        # Selling YES = hit the bid (taker sell)
                        # Selling NO = hit the bid on NO token (already fetched correct token)
                        # ==========================================================
                        if bids:
                            # Use best bid - this is what we'd actually get in real trading
                            best_bid = float(bids[0].get('price', 0))
                            ...
                            # For YES positions, bid price IS the exit price
                            # For NO positions, we fetched NO orderbook, so bid is correct
                            current_price = best_bid
                            market_data['yes_price'] = current_price if side == 'YES' else (1 - current_price)
                            market_data['token_ids'] = token_ids
                            market_data['order_book'] = {'bids': bids, 'asks': asks}
                            
                            logger.info(f"[SPORTS-EXIT] Real orderbook price for {market_id[:16]}: "
                                       f"${current_price:.4f} (bid size: {bid_size:.2f})")
                        else:
                            # No bids = no buyers = cannot exit
            ...
            if bids:
                ...
                # No orderbook available - use mid-price with spread estimate
                ...
                logger.debug(f"[EXIT-EVAL] No orderbook, using estimated exit: {exit_yes_price:.4f}")
