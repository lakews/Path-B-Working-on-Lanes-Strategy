...
const Configuration = () => {
  ...
  const resetToDefaults = () => {
    setConfig({
      ...
      kelly_fraction: 0.25,
      kelly_enabled: true,
      // NEW: Dual Circuit Breaker defaults
      max_account_drawdown_pct: 10,
      max_realized_drawdown_pct: 15,
      min_liquidity: 100,
      max_liquidity: 1000000,
      min_volume_24h: 1000,
      max_spread: 0.25,
  ...
  // NEW: Dual circuit breaker values
  const accountCBValue = (config.initial_capital * (config.max_account_drawdown_pct || 10) / 100);
  const realizedCBValue = (config.initial_capital * (config.max_realized_drawdown_pct || 15) / 100);
